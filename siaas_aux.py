...
def read_mongodb_collection(collection, siaas_uuid="00000000-0000-0000-0000-000000000000"):
    ...
    try:
        ...
        if(siaas_uuid == "00000000-0000-0000-0000-000000000000"):
            ...
            # cursor = collection.find({"payload": {'$exists': True}, "destiny": "agent_"+siaas_uuid}).sort('_id', -1) # show most recent first
            cursor = collection.find({"payload": {'$exists': True}, "destiny": {'$in': [
                                     "agent_"+siaas_uuid, "agent_ffffffff-ffff-ffff-ffff-ffffffffffff"]}}).sort('_id', -1)  # show most recent first
            # cursor = collection.find({"payload": {'$exists': True}}).sort('_id', -1).limit(5)  # show only the 5 most recent
            # cursor = collection.find({"payload": {'$exists': True}, "origin":"server_"+siaas_uuid},{'_id': False}).sort('_id', -1).limit(5) # same, but only for the UUID of this server
